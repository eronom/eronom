...
fn main() {
    ...
    let mir_dir = Path::new(&manifest_dir).join("vendor").join("mir");
    ...
    let libuv_dir = Path::new(&manifest_dir).join("vendor").join("libuv");
    if is_windows {
    ...
    let u_sockets_dir = Path::new(&manifest_dir).join("vendor").join("uWebSockets").join("uSockets");
    ...
    let u_websockets_dir = Path::new(&manifest_dir).join("vendor").join("uWebSockets");
