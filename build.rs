...
fn main() {
    ...
    let mir_dir = Path::new(&manifest_dir).join("native").join("mir");
    ...
    let libuv_dir = Path::new(&manifest_dir).join("native").join("libuv");
    if is_windows {
    ...
    let u_sockets_dir = Path::new(&manifest_dir).join("native").join("uWebSockets").join("uSockets");
    ...
    let u_websockets_dir = Path::new(&manifest_dir).join("native").join("uWebSockets");
