...
fn find_test_files_in_dir(dir: &std::path::Path, out: &mut Vec<std::path::PathBuf>) {
...
pub fn run_test_command(file_opt: Option<std::path::PathBuf>) -> anyhow::Result<()> {
    let test_files = if let Some(path) = file_opt {
        ...
        find_test_files_in_dir(std::path::Path::new("."), &mut files);
        files.sort();
        files.dedup();
        if files.is_empty() {
