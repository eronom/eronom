/// Legacy tree-walking interpreter (pre-VM).
...
pub fn run_file(path: &str) -> anyhow::Result<()> {
    ...
    execute_statements(&lines, &mut variables, &mut if_was_executed, 0)
}

pub fn run_source(source: &str) -> anyhow::Result<()> {
    let mut variables = HashMap::new();
            ...
            if let Some(in_idx) = trimmed.find(" in ") {
                if let Some(dotdot_idx) = trimmed.find("..") {
                    if let Some(brace_idx) = trimmed.find('{') {
                        ...
                        let start_val = evaluate_expression(start_expr, variables)
                            .parse::<i64>()
                            .unwrap_or(0);
                        let end_val = evaluate_expression(end_expr, variables)
                            .parse::<i64>()
                            .unwrap_or(0);
