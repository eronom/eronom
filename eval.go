...
// It replaces goja by handling only what .erm templates actually need:
// variable lookups, dot access, comparisons, and boolean logic.
type ErmEval struct {
...
// Handles signal(val) unwrapping, object literals, arrays, numbers, strings, booleans.
...
// --- Value parser (for variable declarations) ---
...
func parseJSValue(s string, pos int) (interface{}, int) {
	...
	// Unrecognized (function call, identifier expression, etc.) - return nil
	return nil, pos
}
...
func (p *exprParser) parsePrimary() (interface{}, error) {
	...
	if isIdentChar(c) && !(c >= '0' && c <= '9') {
		...
		// Resolve dot access: porridge.temperature
